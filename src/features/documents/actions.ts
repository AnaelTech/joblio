...
export async function uploadDocument(
  ...
  | { success: true; document: { id: string; filename: string; storagePath: string; mimeType: string | null; size: number } }
...
> {
  try {
    ...
    if (file.type && !allowedMimes.includes(file.type)) {
      return { success: false, error: "Type de fichier non autorisé (PDF, images, DOC, DOCX, TXT)" };
    }

    if (file.type && ext) {
      const expectedExts = EXTENSION_MAP[file.type];
      if (expectedExts && !expectedExts.includes(ext)) {
        return { success: false, error: "L'extension du fichier ne correspond pas à son type" };
      }
    }
    ...
    if (file.type && !checkMagicBytes(new Uint8Array(buffer), file.type)) {
      return { success: false, error: "Le contenu du fichier ne correspond pas au type déclaré" };
    }
    ...
    const [doc] = await db
      ...
      .returning({ id: documents.id, filename: documents.filename, storagePath: documents.storagePath, mimeType: documents.mimeType, size: documents.size });
    ...
    return { success: false, error: getErrorMessage(e, "Erreur lors de l'upload") };
  }
}
