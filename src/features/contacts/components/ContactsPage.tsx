...
function ContactsInner({ initialData }: Props) {
	const [selectedId, setSelectedId] = useState<string | null>(null);
	const [toast, setToast] = useState<string | null>(null);
	const filters = useMemo(() => EMPTY_FILTERS, []);
	const { data } = useContacts(filters, initialData);
	...
	return (
		<>
			...
			<ContactModal
				...
				onDeleted={(name) => setToast(`"${name}" a été supprimé`)}
			/>
...
export default function ContactsPage({ initialData }: Props) {
	return (
		<QueryClientProvider client={queryClient}>
			<ContactsInner initialData={initialData} />
		</QueryClientProvider>
	);
}
